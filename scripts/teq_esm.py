...
for ifig in np.arange(len(refwavs)):
    ...
    for ixx in np.arange(2):
        ...
        # output files
        embedfile = f'_includes/{istr}_esm{str(refwavs[ifig])}_' \
                    f'candidates_embed.html'
        fullfile = f'_includes/{istr}_esm{str(refwavs[ifig])}_candidates.html'

        # set up the full output file
        if ixx == 0:
            plotting.output_file(fullfile, title='Equilibrium Temp vs ESM')
        else:
            plotting.output_file(fullfile, title='Insolation vs ESM')
        ...
        for ii, imiss in enumerate(missions):
            ...
            if imiss == 'Other Confirmed':
                good = ((~np.in1d(dfpl['facility'], ['Kepler', 'K2', 'TESS'])) &
                        np.isfinite(dfpl['teq']) & np.isfinite(dfpl[iesm]) &
            ...
            # plot the planets
            # nonselection stuff is needed to prevent planets in that category
            # from disappearing when you click on a data point ("select" it)
            if ixx == 0:
                ...
                glyph = fig.scatter('insolation', 'esm', color=colors[ii],
                                    source=source, size=size, alpha=alpha,
                                    marker=markers[ii], nonselection_alpha=0.07,
                                    selection_alpha=0.8,
                                    nonselection_color=colors[ii])
