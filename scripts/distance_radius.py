...
for ii, imiss in enumerate(missions):
    ...
    if imiss == 'Other Confirmed':
        ...
        source = plotting.ColumnDataSource(data=dict(
                ...
                distance=dfkoi['distance_pc'][good],
        ...
        good = ((dftoi['disp'] == 'Candidate') & np.isfinite(dftoi['prade']) &
                np.isfinite(dftoi['distance_pc']))
        # what the hover tooltip draws its values from
        source = plotting.ColumnDataSource(data=dict(
                planet=dftoi['TOI'][good],
                distance=dftoi['distance_pc'][good],
    ...
    if 'Confirmed' in imiss:
        ...
        source = plotting.ColumnDataSource(data=dict(
                ...
                distance=dfcon['distance_pc'][good],
...
csvhead = '# Planet Name, Status, Period (days), Radius (Earths), ' \
          'Radius (Jupiters), Distance (pc), Discovered by'
keys = ['planet', 'status', 'period', 'radius', 'jupradius', 'distance',
        'discovery']
