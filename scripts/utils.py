...
if True:
    ...
    assert (~np.isfinite(dfcon['st_log_lum']) | ((dfcon['st_log_lum'] > -8) & (dfcon['st_log_lum'] < 5))).all()
    ...
    if new:
        assert badra.sum() == 2 and baddec.sum() == 2 and np.allclose(badra, baddec)
    ...
    assert ((~np.isfinite(dfcon['rade'])) | ((dfcon['rade'] / dfcon['radj'] > 0.99 * radratio) & (dfcon['rade'] / dfcon['radj'] < 1.01 * radratio))).all()
    ...
    for index, icon in dfkoi[koicon].iterrows():
        res = np.where((np.abs(comp['ra'] - icon['ra']) < 1. / 60) &
                       ...
                       (np.abs((comp['period'] - icon['period'])/icon['period']) < 0.01))
        res = res[0]
        if len(res) != 1:
            # special cases I know about that we can match up manually
    ...
    for index, ican in dfkoi[koican].iterrows():
        res = np.where((np.abs(comp['ra'] - ican['ra']) < 1. / 60) &
                       ...
                       (np.abs((comp['period'] - ican['period'])/ican['period']) < 0.01))
        res = res[0]
        if len(res) > 0:
            assert len(res) == 1
    ...
    if updated_koi_params:
        ...
        for index, ican in dfkoi[koican].iterrows():
            ...
            if len(fd) != 1:
                ...
                iau = (((dfkoi.at[index, 'period'] / 365.256)**2) * mm[fd])**(1./3.)
                iinsol = (10.**ll[fd]) * (iau**-2)
        ...
        for index, icon in comp[cononly].iterrows():
            ...
            if len(fd) != 1:
                warnings.warn(f"Can't find parameters for planet {icon['name']}")
            elif ~np.isfinite(mm[fd[0]]):
                continue
            else:
                fd = fd[0]
                ...
                iau = (((comp.at[index, 'period'] / 365.256)**2) * mm[fd])**(1./3.)
                iinsol = (10.**ll[fd]) * (iau**-2)
    ...
    sunearth = 109.1
    rr = np.sqrt(dfkoi['koi_depth'] / 1e6) * dfkoi['st_rad'] * sunearth
    ...
    if updated_k2_params:
        ee, mm, rr, tt, ll, dd = np.loadtxt('data/k2_params_hardegree-ullman2020.txt', unpack=True)
        ee = ee.astype(int)
        ...
        for index, ican in dfk2[k2can].iterrows():
            ...
            if len(fd) != 1:
                ...
                iau = (((dfk2.at[index, 'period'] / 365.256)**2) * mm[fd])**(1./3.)
                iinsol = (10.**ll[fd]) * (iau**-2)
    ...
    # assert k2can.sum() == np.unique(dfk2[dfk2['disposition'] == 'Candidate']['name']).size
    ...
    ichk = ['period', 'rade', 'radj', 'st_rad', 'st_teff', 'st_log_lum',
            'st_mass', 'insol', 'semi_au']
    
    for index, ican in dfk2[k2can].iterrows():
        srch = np.where((dfk2['name'] == ican['name']) & (dfk2['k2c_recentflag'] == 0))[0]
        if len(srch) == 0:
            continue
        for icol in ichk:
    ...
    # everything has a planet and host name
    assert (np.array([len(xi) for xi in dftoi['name']]) > 0).all()
    assert (np.array([len(xi) for xi in dftoi['hostname']]) > 0).all()
    ...
    dftoi.loc[np.where(noper)[0], 'period'] = np.nan
    assert (~np.isfinite(dftoi['period']) | (dftoi['period'] > 0)).all()
    
    assert (~np.isfinite(dftoi['distance_pc']) | (dftoi['distance_pc'] > 1)).all()
    assert (~np.isfinite(dftoi['rade']) | (dftoi['rade'] > 0)).all()
    assert (~np.isfinite(dftoi['insol']) | (dftoi['insol'] > 0)).all()
    ...
    # RA and Dec are both valid
    assert ((dftoi['ra'] >= 0) & (dftoi['ra'] <= 360.)).all()
    assert ((dftoi['dec'] >= -90) & (dftoi['dec'] <= 90.)).all()
    ...
    # XXX: TOI-515 and 844.01 are also K2 planet candidates, so remove it from
    ...
    for index, icon in dftoi[toicon].iterrows():
        res = np.where((np.abs(comp['ra'] - icon['ra']) < 1. / 60) &
                       (np.abs(comp['dec'] - icon['dec']) < 1. / 60) &
                       (np.abs((comp['period'] - icon['period']) / comp['period']) < 0.01))
        res = res[0]
        if len(res) == 0:
    ...
    for index, ican in dftoi[toican].iterrows():
        res = np.where((np.abs(comp['ra'] - ican['ra']) < 1. / 60) &
                       (np.abs(comp['dec'] - ican['dec']) < 1. / 60) &
                       (np.abs((comp['period'] - ican['period']) / comp['period']) < 0.01))
        res = res[0]
    ...
    assert newcan.sum() == toican.sum() + len(waiting) - len(tobeconf) - len(nopermatch)
    assert newcon.sum() == toicon.sum() + len(tobeconf) + len(nopermatch) - len(waiting)
    ...
    assert (~np.isfinite(dftoi['st_rad']) | (dftoi['st_rad'] > 0.05)).all()
    assert (~np.isfinite(dftoi['st_teff']) | (dftoi['st_teff'] > 1000)).all()
    assert (~np.isfinite(dftoi['st_mass']) | (dftoi['st_mass'] > 0.05)).all()
    
    # fill in missing luminosities with our own calculation
    tmplums = (dftoi['st_rad'] ** 2) * ((dftoi['st_teff'] / 5772) ** 4)
    tofill = ~np.isfinite(dftoi['st_log_lum'])
    dftoi.at[tofill, 'st_log_lum'] = np.log10(tmplums[tofill])
    assert ((~np.isfinite(dftoi['st_log_lum'])) | ((dftoi['st_log_lum'] > -8) & (dftoi['st_log_lum'] < 7))).all()
    ...
    dftoi['semi_au'] = iau
    dftoi.at[igd, 'insol'] = iinsol[igd]
    
    assert (~np.isfinite(dftoi['insol']) | (dftoi['insol'] > 0)).all()
    assert (~np.isfinite(dftoi['semi_au']) | (dftoi['semi_au'] > 0)).all()
